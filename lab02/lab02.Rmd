...
### Naïve Bayes Confusion Matrices and Misclassification Rates
...
We can see that the misqualification rate for the optimized decision tree with ```0.2633333``` is better than the Naïve Bayes approach with a rate of ```0.29```. We have to keep in mind that we first had to find the best tree and thus spend more time optimizing the hyper parameters.
...
ggplot(roc_values) +
  ...
  geom_line(aes(x = fprs_bayes, y = tprs_bayes,
                colour = "ROC Naïve Bayes")) +
  geom_point(aes(x = fprs_bayes, y = tprs_bayes), colour = "blue") +
  
  labs(title = "ROC for Optimized Tree and Naïve Bayes", y = "TPR",
       x = "FPR", color = "Legend") +
