...
kernel_gauss_hour = function (hourA, hourB, smoothing) {
  ...
  hourA_in_h = as.numeric(difftime(hourA$time ,
                                   strptime("00:00:00", format = "%H:%M:%S")))
  hourB_in_h = as.numeric(difftime(hourB$time ,
                                   strptime("00:00:00", format = "%H:%M:%S")))
  diff = (hourA_in_h + hourB_in_h) %% 24
...
# Smoothing factors
h_distance = 300000 #1000000 # 5000
h_date = 7 #700 # 7
h_time = 50 #2600 # 26
...
predict_weather = function(u_latitude, u_longtitude, u_date, u_type) {
  ...
  # Smoothing factors
  h_i_distance = 300000 #1000000 # 5000
  h_i__date = 20 #700 # 7
  h_i_time = 50 #2600 # 26
  ...
  for (i in 1:length(times)) {
    ...
    if (u_type == "sum") {
      ...
      kernel = kernel_sum(user_data_point, st, h_i_distance, h_i__date, h_i_time)
    }
    else if (u_type == "prod") {
      # Parameters: A, B, h_distance, h_date, h_time
      kernel =
        kernel_product(user_data_point, st, h_i_distance, h_i__date, h_i_time)
    }
    else {
      stop("Du Nasenbaer!")
    }
