...
Split the spambase into training and validation:
...
```{r}
spambase_model = glm(as.formula("Spam ~ ."), data = train, family = "binomial")
...
```{r}
kknn_model = train.kknn(formula = as.formula("Spam ~ ."), data = train, kmax = 30)

## Train
spambase_predict_kkn_train = predict(object = kknn_model, newdata = train)
spambase_predict_kkn_train = apply(as.matrix(spambase_predict_kkn_train), c(1), FUN = function(x) return(x > 0.5))
confusion_matrix_kkn_train = as.matrix(table(spambase_predict_kkn_train, train$Spam))

## Test
spambase_predict_kkn_test = predict(object = kknn_model, newdata = test)
spambase_predict_kkn_test = apply(as.matrix(spambase_predict_kkn_test), c(1), FUN = function(x) return(x > 0.5))
confusion_matrix_kkn_test = as.matrix(table(spambase_predict_kkn_test, train$Spam))
```
