...
The first step is to calculate $Z$. We will divide that in first calculating $Bx$ which contains $Bernoulli(x|\mu_k)$ and afterwards calculating $p(x)$ which we can use to calculate $Z$. The formulas will have (left side) will be assigned to a letter which you will find in the source code.
...
Voilà we're done, now the coding is actually just a few lines of code, you'll find it in the appendix.
