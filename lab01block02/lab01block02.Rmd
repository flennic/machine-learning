...
$Z$ holds the probability for each set of coin tosses (row in $X$) to belong to each $K_i$. We always choose the one which most likely fits to the observed data and use that to update ouzr priors.

Let's have a look at the mathematical equations and how we can derive our formulas for the matrix multiplication from that

The first step is to calculate $Z$. We will divide that in first calculating $Bx$ which holds $Bernoulli(x|\mu_k)$ and afterwards calculating $p(x)$ which we can use to calculate $Z$. The formales will reduce the left side to single letters which you will find in the source code.


$$Bernoulli(x|\mu_k) = B = \prod_{i} \mu^{x_i}_{k_i} * (1 - \mu_{k_i})^{1-x_i}$$
...
$$B = e^{\sum \ln (\mu^{x_i}_{k_i}) * x_i + \sum \ln (1-\mu_{x_i}) * (1 - x_i)}$$
This craves to be put into
